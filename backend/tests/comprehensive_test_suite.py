...
def test_module_traffic():
    ...
    def get_streets():
        ...
        if status == 200 and isinstance(body, dict) and "data" in body and isinstance(body["data"], list) and len(body["data"]) > 0:
            return status, body, elapsed, None
        return status, body, elapsed, err or "Danh sách đường trống hoặc định dạng sai"
...
def test_module_routing():
    ...
    for idx, (label, f_lat, f_lng, t_lat, t_lng) in enumerate(cases, 1):
        def run_routing_shortest():
            ...
            if status == 200 and "path" in body and len(body["path"]) > 0:
                return status, body, elapsed, None
...
def test_module_incidents():
    ...
    # Email động để tránh trùng lặp tài khoản đã bị vô hiệu hóa từ lần chạy trước
    csgt_email = f"test_csgt_{int(time.time())}@danang-traffic.vn"
    ...
    def login_csgt():
        payload = {"email": csgt_email, "password": "Csgt@2026!"}
        status, body, elapsed, err = request_api("/auth/login", "POST", payload)
        if status == 200 and "access_token" in body:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # test_module_prediction()  # Bỏ qua theo yêu cầu của user do chức năng chưa hoàn thành
    finally:
        cleanup()
